...
for (const required of [node, launcher, path.join(artifact, "app", "node_modules", "custodian", "index.js"), path.join(artifact, "app", "START-HERE.md"), path.join(artifact, "app", "LICENSE"), path.join(artifact, "app", "THIRD_PARTY_NOTICES.md")]) assert.ok(fs.existsSync(required), `missing ${required}`);
...
const completed = run(["--resume", "cert", "--natural", "use my light", "--save", "cert"]); assert.equal(completed.status.lifecycle, "completed");
let dev = startRun({ profile: "field-researcher", seed: "packaged" }).run; act(dev, "MOVE"); act(dev, "INSPECT", look(dev).aliases[0].alias); act(dev, "USE");

...
function expeditionAction(run, verb, target) {
  ...
  if (verb === "RECORD") { const view = look(run); const alias = target ?? view.aliases[0]?.alias; if (!alias || !view.aliases.some((entry) => entry.alias === alias)) return { ok: false, error: { code: "TARGET_UNAVAILABLE" }, run }; const operator = run.spatial ? player : expedition.equipment?.["recording-device"]?.holder ?? player; const device = useEquipment(expedition, "recording-device", operator); if (!device.ok) return { ok: false, error: { code: device.code }, run }; const cost = costFor(verb); const evidence = { id: `field-note-${expedition.evidence.length + 1}`, type: "field-note", creator: player, operator, custodian: player, target_alias: alias, location: view.view?.location ?? null, capture_event: "evidence.recorded", device: "recording-device", storage: "with field record", captured_at: { interval: expedition.clock.interval + cost }, target_observation: "observer-visible target", visible_objects: [alias], provenance: "observer-safe-record", valid: true, available_to_player: true, available_to_standard: false, reporting_state: "unreported", interval: expedition.clock.interval + cost }; evidenceAuthority.capture(run._world, run, evidence); evidence.render = { status: "not-rendered" }; expedition.evidence.push(evidence); event(expedition, "evidence.recorded", evidence); const cycle = resolveOperationalCycle(run, verb, cost); return { ok: true, outcome: "succeeded", result: { public_reason: null, time_advanced: cycle.clock.cost, evidence: { id: evidence.id, type: evidence.type, render_status: evidence.render.status }, mission_updates: cycle.mission_updates, operational_updates: cycle.public_updates }, run }; }

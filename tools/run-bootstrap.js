...
function newRun({ profile, seed, session, expedition, staffing = null, loadout = null, mission = null, procedural_state, procedural_scenario = false, spatial_state = null, object_state = null, survey_frontier = null, spatial_pack_id = null, world_id = null, run_id = null, world = null, phase = "BRIEFING" }) {
  ...
  if (run.spatial_pack_id) {
    ...
    run.survey_frontier = surveyFrontier.migrate(run.survey_frontier, definition, { ...context, spatial: run.spatial, at: run.expedition.clock?.interval ?? 0 });
    surveyFrontier.observe(run.survey_frontier, definition, player, run.spatial.player_location, { at: run.expedition.clock?.interval ?? 0, co_present: Object.entries(run.spatial.personnel_locations).filter(([id, location]) => id !== player && location === run.spatial.player_location).map(([id]) => id) });
...
function look(runValue) {
  ...
  const observer = run.session.startup.player.observer_id;
  if (run.spatial) {
    surveyFrontier.observe(run.survey_frontier, spatialDefinitionFor(run.spatial_pack_id), observer, run.spatial.player_location, { at: run.expedition.clock?.interval ?? 0, co_present: Object.entries(run.spatial.personnel_locations).filter(([id, location]) => id !== observer && location === run.spatial.player_location).map(([id]) => id) });
    observeCurrentObjects(run);
...
function act(runValue, verb, target) {
  ...
  if (verb === "MOVE" && run.spatial) { const context = { ...spatialContext(run), observe_objects: () => objectProjection(run).map((object) => object.observation) }; const moved = spatialRuntime.move(run.spatial, spatialDefinitionFor(run.spatial_pack_id), target, context); if (!moved.ok) return { ok: false, error: { code: moved.code }, result: { public_reason: moved.reason }, public_reason: moved.reason, run }; const player = run.session.startup.player.observer_id; const present = Object.entries(run.spatial.personnel_locations).filter(([id, location]) => id !== player && location === moved.to).map(([id]) => id); surveyFrontier.traverse(run.survey_frontier, spatialDefinitionFor(run.spatial_pack_id), player, moved.connection_id, moved.from, moved.to, { at: run.expedition.clock?.interval ?? 0, co_present: present }); run.checklist.moved = true; observeCurrentObjects(run); event(run.expedition, "spatial.location.entered", { location: moved.to, connection: moved.connection_id, time_cost: moved.time_cost }); const cycle = resolveOperationalCycle(run, verb, moved.time_cost, "spatial-traversal"); return { ok: true, outcome: "succeeded", result: { public_reason: moved.narration, time_advanced: cycle.clock.cost, spatial: { from: moved.from, to: moved.to, connection: moved.connection_id }, mission_updates: cycle.mission_updates, operational_updates: cycle.public_updates }, run }; }
...
module.exports = { startRun, status, look, inspect, act, saveRun, resumeRun, generatorFor, spatialDefinitionFor, interactionDefinitionFor, missionDefinitionFor, dynamicsDefinitionFor, logisticsDefinitionFor, institutionalDefinitionFor, ensureSpatial, setSpatialPhase, enterSpatialField, crossThreshold, objectProjection, evaluateMissionState, resolveOperationalCycle, synchronizeMissionOutcome };

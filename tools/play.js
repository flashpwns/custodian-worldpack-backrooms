...
async function main() {
  let run;
  if (value("--resume")) { const resumed = resumeRun(JSON.parse(fs.readFileSync(value("--resume"), "utf8"))); if (!resumed.ok) throw new Error("unable to restore save"); run = resumed.run; } else run = startRun({ profile: value("--profile") || "field-researcher", seed: value("--seed") || "demo", scenario: value("--scenario") }).run;

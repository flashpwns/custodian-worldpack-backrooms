...
function runReport(name) { if (!reportRegistry[name]) throw failure("REPORT_UNKNOWN", `Unknown report '${name}'. Run 'reports --list'.`); const windows = process.platform === "win32"; const command = windows ? (process.env.ComSpec ?? "cmd.exe") : "npm"; const args = windows ? ["/d", "/s", "/c", `npm run ${name}`] : ["run", name]; const result = spawnSync(command, args, { encoding:"utf8" }); if (result.status !== 0) throw failure("REPORT_FAILED", `Report '${name}' failed (state changed: NO).`); return { version:"yellow-beast-dev-report-run@v1", mutation:"READ_ONLY", name, status:"passed", output:result.stdout.trim() }; }

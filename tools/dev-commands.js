...
function subject(snapshot, target, id, observer, events = []) {
  const objective = snapshot.objective;
  if (target === "world") return objective.world;
  if (target === "observer") { const view = snapshot.observer_views[observer]; if (!view) throw failure("OBSERVER_UNKNOWN", `Unknown observer '${observer}'.`); return view; }
  const groups = { actor:objective.characters, object:[...objective.objects.artifacts, ...objective.objects.evidence], region:objective.regions, phenomenon:objective.phenomena, thread:objective.threads.index.threads ?? [], event:events.slice(-20).map(({ id, sequence, type, run_id, payload }) => ({ id, sequence, type, run_id, payload })) };
  if (!Object.hasOwn(groups, target)) throw failure("INSPECT_TARGET_UNKNOWN", `Unknown inspect target '${target}'.`);
  if (!id) return target === "event" ? { bounded:true, events:groups.event } : groups[target]; const found = groups[target].find((entry) => entry.id === id || entry.identity === id);
  if (!found) throw failure("SUBJECT_UNAVAILABLE", `No ${target} matches '${id}'. State changed: NO.`); return found;
}
...
module.exports = { stable, fixtureRegistry, reportRegistry, inspect, compare, trace, reproduce, threadRebuild, bugBundle, reports, runReport, runReports, providerStatus };

...
function continuityContext(world) {
  ...
  const followUps = (world?.institutional_response?.follow_up_assignments ?? []).filter((item) => item.status === "available");
  return { equipmentLoss, missing, regions, followUps, contactLoss: events.filter((event) => /contact|unresponsive/i.test(event.type)), reports: events.filter((event) => ["q4.communication.reported", "q4.report.filed"].includes(event.type)) };
}
function historyReferences(world) {
  ...
  return [
    ...
    ...context.followUps.map((item) => ({ id: item.id, kind: "institutional-follow-up", status: "authorized follow-up assignment", text: item.public_summary }))
  ];
}
function chooseFamily(world, seed) {
  const context = continuityContext(world);
  if (context.followUps.some((item) => item.id === "q4-accountability-review")) return CATALOG.find((item) => item.id === "contact-loss-investigation");
  if (context.followUps.some((item) => item.id === "q4-evidence-corroboration")) return CATALOG.find((item) => item.id === "recovered-media-follow-up");
...
function generate({ world = null, run_id = null, seed = "yellow-beast-q4", staffing = null } = {}) {
  ...
  const followUp = (world?.institutional_response?.follow_up_assignments ?? []).find((item) => item.status === "available" && ((item.id === "q4-accountability-review" && family.id === "contact-loss-investigation") || (item.id === "q4-evidence-corroboration" && family.id === "recovered-media-follow-up"))) ?? null;

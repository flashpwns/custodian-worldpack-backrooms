...
const DEFINITIONS = Object.freeze({
  "field-researcher": ["BRIEFING", "STAGING", "FACILITY_TRANSIT", "THRESHOLD", "FIELD_OPERATION", "RETURN", "DEBRIEF"],
...
const surfaces = (mode, phase) => mode === "field-researcher" && phase === "BRIEFING" ? ["scene", "mission", "team", "input"] : mode === "field-researcher" ? ["scene", "inventory", "radio", "input"] : ["scene", "context", "input"];
...
function transition(phase, next, { reason = "application-transition", guard = true } = {}) { validate(phase.mode_id, next); if (!guard) return { ok: false, code: "PHASE_GUARD_REJECTED", phase }; const allowed = DEFINITIONS[phase.mode_id]; if (allowed.indexOf(next) < phase.phase_index && phase.mode_id !== "async-command") return { ok: false, code: "INVALID_PHASE_TRANSITION", phase }; const updated = { ...phase, phase_id: next, phase_type: next, phase_index: allowed.indexOf(next), available_ui_surfaces: surfaces(phase.mode_id, next), presentation_profile: `${phase.mode_id}:${next.toLowerCase()}`, history: [...phase.history.slice(-15), { type: "phase_entered", phase_id: next, reason }] }; return { ok: true, phase: updated }; }

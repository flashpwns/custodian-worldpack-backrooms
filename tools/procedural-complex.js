...
function safeAlias(node) { const names = { corridor: "column corridor", junction: "junction", "service-room": "utility room", "stair-transition": "stairwell", staging: "survey boundary" }; return names[node.family] ?? "the immediate area"; }
...
function visible(state, observer) { const current = state.current[observer]; const node = state.nodes[current]; const known = state.discovery[observer] ?? { spaces: [], edges: [], features: [] }; const exits = Object.values(state.edges).filter((edge) => edge.from === current).map((edge, index) => ({ alias: index ? `another passage from ${node.alias}, level ${node.depth + 1}` : `the open passage from ${node.alias}, level ${node.depth + 1}`, edge_id: edge.id, destination_known: edge.to ? known.spaces.includes(edge.to) : false })); return { location: { alias: node.alias, family: node.family, lighting: "local fixture illumination", authority: node.authority }, features: node.features.map((feature) => ({ alias: feature.kind === "fixture" ? "ceiling fixture" : "floor surface", kind: feature.kind })), exits }; }
function resolveAlias(state, observer, alias) { const view = visible(state, observer); const exit = view.exits.find((entry) => entry.alias === alias); if (exit) return { type: "edge", id: exit.edge_id }; const node = state.nodes[state.current[observer]]; const feature = node.features.find((entry, index) => view.features[index]?.alias === alias); if (feature) return { type: "feature", id: feature.id }; return null; }

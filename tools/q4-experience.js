...
function presentation(run, phase, unfinished = null) { const expedition = run.expedition; return { version: VERSION, phase: phase.phase_id, briefing: copy[phase.phase_id], mission: expedition?.order?.primary ?? null, display_mission: "Review the bounded field survey and return with a field record.", restrictions: expedition?.order?.constraints ?? [], reporting: expedition?.order?.reporting ?? null, team: expedition?.team?.members.map(({ id, role, status }) => ({ name: id === "yb-field-peer-observer" ? "Field researcher" : "You", role, status })) ?? [], equipment: Object.entries(expedition?.equipment ?? {}).map(([id, item]) => ({ label: id.replace(/-/g, " "), state: item.state, charges: item.charges })), radio: expedition?.messages?.slice(-1).map(({ intended_recipient, delivery_status }) => ({ recipient: intended_recipient === "Standard" ? "Operations" : "team", delivery_status })) ?? [], operational_follow_up: (unfinished?.items ?? []).filter((item) => ["communication", "observation", "personnel", "report", "object"].includes(item.kind)), human_context: humanWorld.q4Context() }; }

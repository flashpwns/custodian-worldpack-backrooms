...
function localModule(fromRelative, request) {
  ...
  if (!resolved || path.relative(root, resolved).startsWith("..")) throw new Error(`Unable to resolve packaged local module '${request}' from '${fromRelative}'.`);
  return path.relative(root, resolved);
}
function copyRuntimeClosure(entries) {
  const pending = Array.isArray(entries) ? [...entries] : [entries]; const copied = new Set();
  while (pending.length) {
    const relative = pending.pop(); if (copied.has(relative)) continue;
    copied.add(relative); copy(relative);
    if (path.extname(relative) !== ".js") continue;
    const source = fs.readFileSync(path.join(root, relative), "utf8");
    for (const match of source.matchAll(/require\(\s*["']([^"']+)["']\s*\)/g)) {
      const dependency = localModule(relative, match[1]); if (dependency && !copied.has(dependency)) pending.push(dependency);
    }
  }
  return [...copied].sort();
}
...
["manifest.json", "LICENSE", "THIRD_PARTY_NOTICES.md", "START-HERE.md", "package.json", "profiles", "scenarios/threshold-baseline.json", "data", "SIMULATION_DOCTRINE.md", "docs/YELLOW_BEAST_1.0_DESIGN_CHARTER.md", "docs/ai-interface-contract.md", "docs/ai-provider.md", "canon/terminology.json", "canon/operational-spatial-schema.json", "canon/operational-interaction-schema.json", "canon/operational-mission-schema.json", "canon/operational-dynamics-schema.json", "canon/operational-logistics-schema.json", "canon/institutional-response-schema.json", "canon/complete-operation-schema.json", "node_modules"].forEach(copy);
...
console.log(JSON.stringify({ artifact, platform: requested, architecture, bundled_node: nodeVersion, custodian: require("custodian/package.json").version, runtime_files: runtimeFiles.length }, null, 2));

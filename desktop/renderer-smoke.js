...
async function run(windowRef) {
  ...
  const waitFor = async (selector, present = true) => { for (let i = 0; i < 80; i += 1) { if (Boolean(await windowRef.webContents.executeJavaScript(`Boolean([...document.querySelectorAll(${JSON.stringify(selector)})].some((n) => n.offsetWidth > 0 && n.offsetHeight > 0) || document.querySelector(${JSON.stringify(selector)}))`)) === present) return true; await pause(25); } return false; };
  const rect = async (selector) => windowRef.webContents.executeJavaScript(`(() => { const nodes = [...document.querySelectorAll(${JSON.stringify(selector)})]; const node = nodes.find((n) => { const r = n.getBoundingClientRect(); const top = document.elementFromPoint(r.left + r.width/2, r.top + r.height/2); return r.width > 0 && r.height > 0 && (top === n || n.contains(top)); }) || nodes.find((n) => { const r = n.getBoundingClientRect(); return r.width > 0 && r.height > 0 && r.top >= 0 && r.top < window.innerHeight; }) || nodes[0] || null; if(!node) return null; node.scrollIntoView({block:"center", inline:"nearest"}); const r=node.getBoundingClientRect(); return {x:r.left+r.width/2,y:r.top+r.height/2,width:r.width,height:r.height,tag:node.tagName,disabled:node.disabled??false,pointer:getComputedStyle(node).pointerEvents,top:document.elementFromPoint(r.left+r.width/2,r.top+r.height/2)?.tagName}; })()`);
  const click = async (selector) => { assert.equal(await waitFor(selector), true, `not present: ${selector}`); const r = await rect(selector); assert.ok(r && r.width > 0 && r.height > 0, `not hit-testable: ${selector}`); assert.equal(r.pointer, "auto", `pointer events blocked: ${selector}`); assert.notEqual(r.top, "BODY", `surface not attached: ${selector}`); await windowRef.webContents.sendInputEvent({ type:"mouseMove", x:Math.round(r.x), y:Math.round(r.y) }); await windowRef.webContents.sendInputEvent({ type:"mouseDown", x:Math.round(r.x), y:Math.round(r.y), button:"left", clickCount:1 }); await windowRef.webContents.sendInputEvent({ type:"mouseUp", x:Math.round(r.x), y:Math.round(r.y), button:"left", clickCount:1 }); await pause(80); };
  ...
  await type('#natural-form [name="text"]', naturalInput);
  await click('#natural-form button[type="submit"]');
  const expectedNaturalLocation = "Columned Corridor";
  ...
  for (let index = 0; index < 120; index += 1) {
    ...
    if (afterNatural.projection.q4.current_location.name === expectedNaturalLocation) break;
    await pause(50);
  }

...
async function run(windowRef, service) {
  ...
  // Native keyboard type-ahead on the focused, closed select. On Windows the click opens Chromium's popup,
  // where type-ahead only highlights an option, so ESCAPE closes it first (a no-op where none opened).
  const sendKey = async (keyCode, char = false) => { await windowRef.webContents.sendInputEvent({ type:"keyDown", keyCode }); if (char) await windowRef.webContents.sendInputEvent({ type:"char", keyCode }); await windowRef.webContents.sendInputEvent({ type:"keyUp", keyCode }); };
  await click('[data-testid="q4-comms-target"]'); await sendKey("ESCAPE"); await pause(30); await sendKey(targetKey, true); await pause(30);

...
async function run(windowRef) {
  ...
  await click('[data-action="personnel-confirm-continue"]'); assert.equal(await waitFor('[data-testid="play-shell"]'), true); await click('[data-game-action="READY"]'); await click('[data-game-action="PROCEED"]'); await click('[data-game-action="APPROACH"]'); await click('[data-game-action="READY"]'); assert.equal(await waitFor('#q4-comms-form'), true);
  ...
  for (let index = 0; index < 120; index += 1) {
    ...
    if (afterNatural.projection.q4.current_location.name === "Open Passage") break;
    await pause(50);
  }
  const expectedNaturalLocation = process.argv.includes("--reference-expedition") ? "Open Passage" : "Columned Corridor";

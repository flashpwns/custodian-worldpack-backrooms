...
class DesktopService {
  ...
  async presentHostedLocalDialogue({ world_id }, canonical, { configured }) {
    ...
    try {
      ...
      if (!validation.ok) {
        ...
        context.interaction.presentation.response = canonical.result.public_reason;
  ...
  submitQ4CommunicationCanonical({ world_id, channel, text, target = null }) {
    ...
    try {
      ...
      if (channel === "local") {
        ...
        const response = responses.map((item) => item.text).filter(Boolean).join(" ");
        ...
        const authorized = addressed?.reaction && addressed.text ? addressed : responses.find((item) => item.reaction && item.text);
  ...
  async submitNatural({ world_id, mode, text }) {
    try {
      ...
      const hostedFailure = configured && this.interpretationProvenance.some((record) => record.request_id === requestId && record.provider === "openai" && record.invocation_status === "failed");

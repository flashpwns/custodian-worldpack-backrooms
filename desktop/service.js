...
class DesktopService {
  ...
  async submitNatural({ world_id, mode, text }) {
    ...
    try { const world = this.getWorld(world_id); const entry = this.session(world_id, mode) ?? this.restoreSession(world, mode, readJson(this.sessionFile(world_id, mode), null)); if (!entry || entry.kind !== "bootstrap") return publicError("SESSION_NOT_FOUND", "Start or continue this session first."); const key = this.credentials.get("openai"); if (!key) return publicError("PROVIDER_CONFIGURATION_REQUIRED", "OpenAI needs a key. Your world is safe; you can continue offline."); const provider = createOpenAIProvider({ apiKey: key, model: this.settings().openai_model || undefined, timeout: 15000 }); const result = await executeNatural({ run: entry.run, provider, player_text: text, request_id: `desktop-natural-${world_id}-${Date.now()}` }); if (result.consequence?.result.accepted) this.persistSession(world, mode, entry); return { ok: true, result: { intent_status: result.intent.status, clarification_required: result.grounded_intent?.clarification_required ?? result.intent.clarification_required, clarification_question: result.grounded_intent?.clarification?.question ?? null, interpretation_error: result.intent.error?.code ?? null, grounded: Boolean(result.grounded_intent?.grounded_references.length), executed: Boolean(result.consequence?.result.accepted), summary: result.consequence?.result.observer_safe_summary ?? null }, projection: this.projectionFor(world, mode, entry) }; } catch (error) { this.log(`provider unavailable: ${error.message}`); return publicError("PROVIDER_UNAVAILABLE", "AI response unavailable. Your world is safe. Continue using structured controls or try again."); }
  }

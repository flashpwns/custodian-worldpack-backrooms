...
class DesktopService {
  ...
  removeProviderKey({ provider = "openai" } = {}) {
    if (!Object.hasOwn(PROVIDER_SPECS, provider)) return publicError("PROVIDER_INVALID", "Choose a supported provider.");
    this.credentials.remove(provider); this.providerPool.resetHealth(provider);
    const next = { ...this.settings() };
    // Removing a selected hosted key must not silently switch gameplay offline.
    if (next.provider === provider) next.provider = "auto";

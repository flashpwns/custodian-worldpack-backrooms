...
const DEFAULT_SETTINGS = Object.freeze({ version: 2, input_mode: "structured", provider: "offline", theme: "system", reduced_motion: false, guided_introductions: true, reopen_last_world: true, mode_onboarding: {} });
...
class DesktopService {
  ...
  projectionFor(world, mode, entry) { const descriptor = this.getMode(mode); const runId = entry.run_id ?? entry.run?.run_id ?? null; let surface;
    ...
    const phase = entry.phase ?? phases.createPhase({ mode, guided: this.settings().guided_introductions !== false }); return { version: "yellow-beast-desktop-projection@v1", world: this.worldInfo(world, this.metadata().worlds[world.world_id] ?? {}), mode: clone(descriptor), gameplay: gameplay.projection(world, { mode: descriptor.gameplay_mode, run_id: runId }), institution: mode === "async-command" ? desk.projection(world) : null, surface: clone(surface), phase: clone(phase), q4: entry.kind === "bootstrap" ? q4.presentation(entry.run, phase) : null, beck: entry.kind === "beck" ? beckExperience.presentation(world, surface, phase) : null, nullzone: entry.kind === "nullzone" ? nullzoneExperience.presentation(world, phase, surface) : null, lost: entry.kind === "lost" ? lostExperience.presentation(surface, phase) : null, scene: this.sceneFor(entry, mode), available_actions: this.availableFor(world, mode, entry), settings: this.settings() };
  }

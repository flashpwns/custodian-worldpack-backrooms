...
class DesktopService {
  constructor({ appDataPath = null, paths = null, logger = null, credentials = null, evidenceMediaProviders = {}, developerMode = process.env.YELLOW_BEAST_DEVELOPER_MODE === "1" } = {}) {
  ...
  recordInterpretationProvenance(record) { this.interpretationProvenance.push({ ...record, execution_mode: record.provider === "deterministic-mock" ? "offline" : (record.provider ?? "unknown"), provider_invoked: true, response_classification: record.provider === "deterministic-mock" ? "deterministic" : "model-generated", authority_registry: this.authorityRegistry?.version ?? null, authority_context_order: ["simulation-doctrine", "worldpack-and-domain-authority", "canonical-current-state", "observer-safe-projection", "persisted-history", "player-submission", "response-contract"], authority_sources: this.authorityRegistry?.sourceMetadata?.() ?? [], recorded_at: new Date().toISOString() }); if (this.interpretationProvenance.length > 100) this.interpretationProvenance.shift(); }
  ...
  getProviderStatus() { const selected = this.settings().provider; const configured = this.credentials.configured("openai"); const settings = this.settings(); const media = this.evidenceMedia.status(settings); return { ok: true, provider: { selected, offline: selected === "offline", openai: { configured, status: selected === "openai" ? (configured ? "ready" : "configuration-required") : "inactive" }, local_provider: { supported: true, adapter: settings.visual_adapter, endpoint: settings.comfyui_endpoint, status: media.local.selected ? (media.local.available ? "ready" : "unavailable") : "inactive" }, evidence_media: media } }; }
  ...
  restoreSession(world, mode, saved) { if (this.validateSessionSave(saved, mode)) return null; const phase = saved.phase ?? phases.createPhase({ mode, guided: this.settings().guided_introductions !== false }); if (saved.kind === "bootstrap") { const current = saved.payload?.version === "yellow-beast-save@v9"; if (currentClearQ4(saved.payload) && !hasCurrentStandardOperator(world)) return null; const hadSpatialState = Boolean(saved.payload?.spatial); const hadRadioState = Boolean(saved.payload?.expedition?.radio); const hadOperationalState = Boolean(saved.payload?.expedition?.operational); const restored = bootstrap.resumeRun(saved.payload, { world, spatial_worldpack: mode === "field-researcher" ? "clear-q4" : null, phase: phase.phase_id }); if (!restored.ok) return null; if (!current) { bootstrap.ensureSpatial(restored.run, phase.phase_id); if (!hadSpatialState) { if (["FIELD_OPERATION", "RETURN", "DEBRIEF"].includes(phase.phase_id)) bootstrap.enterSpatialField(restored.run); else bootstrap.setSpatialPhase(restored.run, phase.phase_id); } if (!hadRadioState && ["FIELD_OPERATION", "RETURN", "DEBRIEF"].includes(phase.phase_id)) q4Radio.completeCheck(restored.run.expedition); if (!hadOperationalState && restored.run.expedition?.clock?.check_in_due_at != null && !restored.run.expedition.communications?.check_ins?.length) q4Time.schedule(restored.run.expedition, Math.max(1, restored.run.expedition.clock.check_in_due_at - restored.run.expedition.clock.interval)); bootstrap.evaluateMissionState(restored.run, phase.phase_id); } return { kind: "bootstrap", run: restored.run, legacy_flow: saved.legacy_flow === true || phase.legacy_flow === true, restored_from_legacy:!current, phase }; } if (saved.kind === "lost") return { kind: "lost", run: saved.payload, phase }; if (saved.kind === "nullzone") return { kind: "nullzone", run_id: saved.payload.run_id, phase }; if (saved.kind === "beck") return { kind: "beck", run_id: saved.payload.run_id, phase }; return null; }
  ...
  commitPersistencePair(world, mode, serialized) {
    const normalizedWorld = history.normalizeWorld(world); const safeSession = history.canonicalJson(serialized, { code:"SESSION_VALUE_INVALID" });
    if (this.validateSessionSave(safeSession, mode) || !this.restoreSession(normalizedWorld, mode, safeSession)) throw Object.assign(new Error("coordinated session candidate failed validation"), { code:"SESSION_SAVE_INVALID" });
    ...
    try {
      ...
      fs.writeFileSync(stagedSession, `${JSON.stringify(safeSession, null, 2)}\n`); const staged = readJsonCandidate(stagedSession); if (!staged.ok || this.validateSessionSave(staged.value, mode) || !this.restoreSession(normalizedWorld, mode, staged.value)) throw Object.assign(new Error("staged session failed validation"), { code:"SESSION_SAVE_INVALID" });
  ...
  persistSession(world, mode, entry) { const candidateWorld = history.canonicalJson(world, { code:"CANONICAL_WORLD_VALUE_INVALID" }); const candidateEntry = sessionRuntimeCandidate(entry); if (candidateEntry.kind === "bootstrap") missionRuntime.attachCompatibilityView(candidateEntry.run.expedition); this.reconcileSessionCandidate(candidateWorld, candidateEntry); const serialized = this.serializeSession(candidateWorld, mode, candidateEntry); const committed = this.commitPersistencePair(candidateWorld, mode, serialized); committed.entry = candidateEntry; this.adoptPersistencePair(world, entry, committed); this.sessions.set(`${world.world_id}:${mode}`, entry); try { const data = this.metadata(); if (data.worlds[world.world_id]) { data.worlds[world.world_id].last_played_at = new Date().toISOString(); data.worlds[world.world_id].last_mode = mode; data.last_world_id = world.world_id; this.writeMetadata(data); } } catch (error) { this.log(`persistence metadata update failed after coordinated commit: ${error.message}`); } }
  persistTerminalRetirement(world, mode, entry) { const candidateWorld = history.canonicalJson(world, { code:"CANONICAL_WORLD_VALUE_INVALID" }); const candidateEntry = sessionRuntimeCandidate(entry); if (candidateEntry.kind === "bootstrap") { candidateEntry.run._world = candidateWorld; missionRuntime.attachCompatibilityView(candidateEntry.run.expedition); } const retired = outcomes.retire(candidateWorld, candidateEntry.run); if (!retired.ok) throw Object.assign(new Error("terminal retirement rejected"), { code:retired.code }); const serialized = this.serializeSession(candidateWorld, mode, candidateEntry); const committed = this.commitPersistencePair(candidateWorld, mode, serialized); committed.entry = candidateEntry; this.adoptPersistencePair(world, entry, committed); this.sessions.set(`${world.world_id}:${mode}`, entry); try { const data = this.metadata(); data.legacy_personnel_archive ??= {}; if (retired.legacy) data.legacy_personnel_archive[retired.legacy.id] = clone(retired.legacy); if (data.worlds[world.world_id]) data.worlds[world.world_id].last_mode = mode; this.writeMetadata(data); } catch (error) { this.log(`retirement metadata update failed after coordinated commit: ${error.message}`); } this.saveSession(world.world_id, mode, serialized, { validateOnly:true }); return retired; }
  ...
  resumeSession({ world_id, mode }) { try { const world = this.getWorld(world_id); if (outcomes.isRetired(world)) return publicError("WORLD_RETIRED", "This world is retired. Its historical archive remains available."); const loaded = this.loadSession(world, mode); if (!loaded.ok) { const messages = { SESSION_NOT_FOUND: "There is no operational record to continue.", SESSION_VERSION_UNSUPPORTED: "This operation record was created by a newer unsupported version and was left unchanged.", SESSION_SCHEMA_UNSUPPORTED: "This operation record cannot be opened safely by this version and was left unchanged.", SESSION_SAVE_DAMAGED: "This operation record is damaged and no verified previous record is available. It was left unchanged." }; return publicError(loaded.code, messages[loaded.code] ?? "This operation record could not be resumed safely."); } if (mode === "field-researcher" && loaded.entry.kind === "bootstrap" && loaded.entry.restored_from_legacy) { standardOperator.ensure(world, loaded.entry.run.run_id); this.saveCanonical(world); } this.sessions.set(`${world_id}:${mode}`, loaded.entry); return { ok: true, session: { world_id, mode, resumable: true }, recovery: { world: this.recoveryStatus(world_id, "world"), session: loaded.recovery }, projection: this.projectionFor(world, mode, loaded.entry) }; } catch { return publicError("SESSION_RESUME_FAILED", "This session could not be resumed safely."); } }
  ...
  submitQ4Communication({ world_id, channel, text, target = null }) {
    ...
    try {
      ...
      if (channel === "local") {
        ...
        const interaction = q4Interactions.record(expedition, { channel, speaker: "You", targets: recipients.map((item) => item.display_name), player_text: message, attempted_behavior: "speak with nearby participating personnel", eligibility: "eligible", delivery: "heard", time_cost: 0, canonical_effects: ["communication.local.delivered"], presentation: { result: "heard", response } });
        ...
        return { ok: true, result: { outcome: "delivered", public_reason: publicReason, message: { id: deliveredLocal.message.id, state: deliveredLocal.message.state }, mission_updates: cycle.mission_updates, scene }, projection: this.projectionFor(world, "field-researcher", entry) };
      }
  ...
  async submitNatural({ world_id, mode, text }) {
    try {
      ...
      const phaseBefore = entry.phase?.phase_id; const interpretationContext = nonBootstrap ? this.naturalContext(world, mode, entry) : this.q4InterpretationContext(world, entry); const requestId = `desktop-natural-${world_id}-${Date.now()}`; const contextHash = crypto.createHash("sha256").update(JSON.stringify(interpretationContext)).digest("hex"); this.recordInterpretationProvenance({ request_id: requestId, route: "submitNatural", input: "player-supplied", provider: provider.name, model: provider.model ?? "deterministic", doctrine: interpretationContext.authority_contract?.doctrine_sha256 ?? null, context_sha256: contextHash, context_sections: Object.keys(interpretationContext), canonical_resolution: "executePlayerTurn -> consequenceResolver", observer_projection: nonBootstrap ? "naturalContext" : "q4InterpretationContext" }); const turn = await executePlayerTurn({ run: adapterRun, mode, provider, player_text: text, request_id: requestId, context: interpretationContext, consequenceResolver: entry.kind === "bootstrap" ? ({ plan }) => this.resolveQ4Attempt({ world_id, mode, entry, plan }) : ({ plan }) => resolveModeAttempt({ service: this, world_id, mode, plan, available }), sceneBuilder: entry.kind === "bootstrap" ? ({ natural: resolved }) => this.sceneFor(entry, mode, { scene_type: resolved.consequence?.result?.accepted ? "delta" : "observation", accepted: resolved.consequence?.result?.accepted, public_reason: resolved.consequence?.result?.observer_safe_summary }, world) : ({ natural: resolved }) => this.modeScene(world, mode, entry, resolved) });

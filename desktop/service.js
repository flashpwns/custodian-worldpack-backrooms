...
class DesktopService {
  ...
  testProvider({ provider = null } = {}) { const status = this.getProviderStatus().provider; const target = provider || status.selected; if (target === "offline") return { ok: true, status: "ready", message: "Offline structured play is ready." }; if (target === "auto") { const candidates = this.providerPool?.getCandidates({ preferredProvider: "auto" }) ?? []; const hosted = candidates.filter((c) => c !== "offline"); return { ok: true, status: hosted.length ? "ready" : "offline-only", message: hosted.length ? `Auto provider is ready. Active chain: ${candidates.join(" -> ")}` : "No hosted providers configured. Auto will use Offline Interpreter." }; } if (!this.credentials.configured(target)) return publicError("PROVIDER_CONFIGURATION_REQUIRED", `${target} needs a key. You can continue offline at any time.`); return { ok: true, status: "ready", message: `${target} is configured. Connection is tested when you choose natural-language input.` }; }
  ...
  submitQ4CommunicationCanonical({ world_id, channel, text, target = null }) {
    ...
    try {
      ...
      if (channel === "local") {
        ...
        const addressedRecipient = addressed ?? (namedInMessage ? responses.find((item) => item.recipient.personnel_id === (namedInMessage.personnel_id ?? namedInMessage.id)) : null);
        const chosen = addressedRecipient ?? responses.find((item) => item.reaction && item.text) ?? responses[0];

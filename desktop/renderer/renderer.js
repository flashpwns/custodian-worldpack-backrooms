...
async function settings(invoker = document.activeElement, feedback = "") {
  ...
  app.querySelectorAll("[data-provider-test]").forEach(item => item.addEventListener("click", async () => {
    ...
    try { const tested = await yellowBeast.testProvider({ provider:item.dataset.providerTest, live:true }); await settings(undefined, tested.ok ? tested.message : tested.error.message); }
  ...
  app.querySelectorAll("[data-provider-delete]").forEach(item => item.addEventListener("click", async () => {
    const removed = await yellowBeast.removeProviderKey({ provider:item.dataset.providerDelete });
    await settings(undefined, removed.ok ? (removed.configured ? "Saved key removed; an environment credential is still available." : "Saved key deleted. Other provider keys were preserved.") : removed.error.message);
  }));
  app.querySelector("[data-provider-auto]").addEventListener("click", async () => {
    ...
    await settings(undefined, saved.ok ? "AUTO fallback order enabled. Only configured hosted providers can interpret live turns." : saved.error.message);
  });
  app.querySelector("[data-provider-save-model]").addEventListener("click", async () => {
    ...
    await settings(undefined, saved.ok ? "Model saved for the selected provider. Use Test connection to verify it." : saved.error.message);
  });
  document.querySelector("#openai").addEventListener("submit", async (event) => {
    ...
    if (saved.ok) {
      ...
      await settings(undefined, `Key stored for ${targetProvider}. ${saved.provider.persistent ? "Encrypted storage is active." : "Key is available for this session only."} Use Test connection to verify it.`);
    }
  });

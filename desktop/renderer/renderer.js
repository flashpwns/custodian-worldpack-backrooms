...
function play(message = "", state = "") {
  ...
  const prefieldAction = q4Prefield ? `<section class="action-dock natural-action prefield-action" data-testid="prefield-primary"><div><p class="eyebrow">CURRENT DECISION</p><h2>${escape(prefieldDirect ? YBSurfaces.actionLabel(prefieldDirect.type) : "Establish the required Standard exchange")}</h2></div>${prefieldDirect ? `<button type="button" class="primary-action" data-game-action="${escape(prefieldDirect.type)}">${escape(YBSurfaces.actionLabel(prefieldDirect.type))}</button>` : `<p>Use STANDARD in the communications panel to continue.</p>`}<p>${prefieldDirect ? "This advances the recorded expedition phase." : "No physical turn is available until the radio procedure is complete."}</p></section>` : "";
...
async function settings(invoker = document.activeElement) {
  ...
  document.querySelector("#provider-config")?.addEventListener("submit", async (event) => { event.preventDefault(); const data = new FormData(event.currentTarget); const targetProvider = data.get("credential_provider"); const saved = await yellowBeast.configureProvider({ provider:targetProvider, api_key:data.get("api_key"), model:data.get("model") || null }); message.textContent = saved.ok ? `${targetProvider} key stored. It is never shown here again.` : saved.error.message; if (saved.ok) settings(); });

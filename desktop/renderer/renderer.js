...
const providerError = (result) => /^PROVIDER_|^OPENAI_/.test(result?.error?.code ?? result?.result?.provider_error?.code ?? "");
...
function renderMessage(result, natural) {
  ...
  if (detail.provider_unavailable) return detail.public_reason ?? detail.provider_error?.message ?? result?.error?.message ?? "The selected AI provider is unavailable. No generated response was substituted.";
...
async function submitTurn(kind, request) {
  ...
  if (resultIsError(result)) {
    if (typeof YBAudio !== "undefined") YBAudio.emitHook("ui_error");
    if (result.projection) current.projection = result.projection;
    if (providerError(result)) { play(result.error?.message ?? renderMessage(result, kind === "natural"), "provider-error"); return; }
  ...
  const previousFacilityOperations = current.projection?.q4?.facility_operations;
  current.projection = result.projection;
  if (result.result?.scene) current.projection.scene = result.result.scene;
  playFacilityOperationAudio(previousFacilityOperations, current.projection?.q4?.facility_operations);
...
function playFacilityOperationAudio(previous, next) {
  if (typeof YBAudio === "undefined") return;
  const priorIds = new Set((previous?.events ?? []).map((item) => item.id));
  for (const item of next?.events ?? []) {
    if (priorIds.has(item.id)) continue;
    if (item.type === "THRESHOLD_CROSSING") YBAudio.emitHook("threshold_cross_hum");
    else if (item.type === "EAST_BLAST_DOOR_OPEN") {
      YBAudio.emitHook("blast_door_release");
      YBAudio.emitHook("blast_door_open");
      YBAudio.emitHook("blast_door_open_stop");
    } else if (item.type === "EAST_BLAST_DOOR_CLOSED") {
...
async function settings(invoker = document.activeElement) {
  ...
  const message = document.querySelector("#settings-message"); const preview = () => applyPreferences({ ...result.settings, theme:theme.value, text_scale:textScale.value, reduced_motion:reducedMotion.checked, guided_introductions:guided.checked }); [theme, textScale, reducedMotion, guided].forEach((item) => item.addEventListener("change", preview)); form.addEventListener("submit", async (event) => { event.preventDefault(); if (settingsController.state === "saving") return; settingsController.state = "saving"; const data = new FormData(form); form.querySelectorAll("input,select,button").forEach((item) => { item.disabled = true; }); const saved = await yellowBeast.updateSettings({ settings:{ input_mode:data.get("input_mode"), provider:data.get("provider"), theme:data.get("theme"), text_scale:data.get("text_scale"), reduced_motion:data.get("reduced_motion") === "on", guided_introductions:data.get("guided_introductions") === "on", visual_rendering:data.get("visual_rendering") === "on", automatic_evidence_rendering:data.get("automatic_evidence_rendering") === "on", visual_adapter:data.get("visual_adapter"), visual_quality:data.get("visual_quality"), retry_failed_renders:data.get("retry_failed_renders") === "on", openai_model:data.get("openai_model") || undefined } }); form.querySelectorAll("input,select,button").forEach((item) => { item.disabled = false; }); if (!resultIsError(saved)) { settingsController.state = "saved"; applyPreferences(saved.settings); message.textContent = "Preferences saved and applied."; } else { settingsController.state = "open"; message.textContent = saved.error.message; } });
...
// Keep provider model configuration in the same persisted Settings transaction.
// The model control is rendered in the optional credentials section, but it is
// still part of the stable Settings surface and must survive reopen.
document.addEventListener("formdata", (event) => {
  if (event.target?.id !== "settings") return;
  const model = document.querySelector('#openai [name="model"]');
  if (model?.value) event.formData.set("openai_model", model.value);
});
const settingsModelObserver = new MutationObserver(() => {
  const model = document.querySelector('#openai [name="model"]');
  if (!model || model.dataset.loaded === "true") return;
  model.dataset.loaded = "true";
  yellowBeast.getSettings().then((result) => {
    if (!resultIsError(result) && result.settings?.openai_model) model.value = result.settings.openai_model;
  }).catch(() => {});
});
settingsModelObserver.observe(app, { childList: true, subtree: true });

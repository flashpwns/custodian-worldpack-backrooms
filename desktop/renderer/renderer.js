...
async function home() {
  ...
  const list = worlds.worlds.map((world) => `<li><strong>${escape(world.name)}</strong><span>${escape(world.last_mode ?? "Ready to choose an experience")}</span><span class="muted">Last played ${escape(world.last_played_at ? new Date(world.last_played_at).toLocaleDateString() : "not yet")}</span>${button("Open world", `world:${world.id}`)}${button("Export", `export:${world.id}`)}${button("Delete", `delete:${world.id}`)}</li>`).join("") || "<li>No worlds yet. Create a world to begin.</li>";
  app.innerHTML = `<section class="shell" data-testid="world-library"><header><div><p class="eyebrow">YELLOW BEAST <span>ALPHA</span></p><h1>World Library</h1><p>Choose a world. Each experience reveals a different legitimate part of the same history.</p></div><p class="version">v${escape(info.app.version)}</p></header><nav aria-label="Application">${button("Create world", "new")}${button("Import world", "import")}${button("Settings", "settings")}${button("About", "about")}</nav><section><h2>Your worlds</h2><ul class="worlds">${list}</ul></section></section>`;
}
function newWorld() {
...
async function enterMode(mode) {
  ...
  current.projection = result.projection; play();
}
function play(message = "", state = "") {
  ...
  document.querySelector("#natural-form").addEventListener("submit", (event) => { event.preventDefault(); const data = new FormData(event.currentTarget); submitTurn("natural", () => yellowBeast.submitNatural({ world_id:current.world.id, mode:current.mode, text:data.get("text") })); });

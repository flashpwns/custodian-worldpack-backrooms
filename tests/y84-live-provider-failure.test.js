...
test("failed key replacement preserves the prior saved key and reports the replacement as session only", t => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "yb-key-replace-"));
  t.after(() => fs.rmSync(root, { recursive:true, force:true }));
  let encryptionFails = false;
  const safeStorage = { isEncryptionAvailable:() => true,
    encryptString:value => { if (encryptionFails) throw Error("Encryption unavailable"); return Buffer.from(value.split("").reverse().join("")); },
    decryptString:value => value.toString().split("").reverse().join("") };
  const store = new CredentialStore({ directory:root, safeStorage });
  assert.equal(store.set("groq", "prior-fixture-key").persistent, true);
  const saved = fs.readFileSync(store.filePathFor("groq"));
  encryptionFails = true;
  assert.equal(store.set("groq", "replacement-fixture-key").persistent, false);
  assert.equal(store.get("groq"), "replacement-fixture-key");
  assert.equal(store.describe("groq").persistent, false);
  assert.equal(store.describe("groq").session_only, true);
  assert.deepEqual(fs.readFileSync(store.filePathFor("groq")), saved);
  assert.deepEqual(fs.readdirSync(root), ["groq.bin"]);
  assert.equal(new CredentialStore({ directory:root, safeStorage }).get("groq"), "prior-fixture-key");
});

test("failed key deletion is explicit and leaves credentials and preferences intact", t => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "yb-key-delete-"));
  t.after(() => fs.rmSync(root, { recursive:true, force:true }));
  const safeStorage = { isEncryptionAvailable:() => true, encryptString:value => Buffer.from(value), decryptString:value => value.toString() };
  const credentials = new CredentialStore({ directory:path.join(root, "credentials"), safeStorage });
  const service = new DesktopService({ appDataPath:root, credentials });
  service.configureProvider({ provider:"groq", api_key:"groq-fixture", model:"groq-model" });
  const before = fs.readFileSync(service.settingsFile, "utf8");
  const unlink = fs.unlinkSync;
  const failure = t.mock.method(fs, "unlinkSync", file => {
    if (file === credentials.filePathFor("groq")) throw Object.assign(new Error("fixture denied"), { code:"EACCES" });
    return unlink(file);
  });
  const result = service.removeProviderKey({ provider:"groq" });
  assert.equal(result.ok, false);
  assert.equal(result.error.code, "CREDENTIAL_REMOVE_FAILED");
  assert.equal(credentials.configured("groq"), true);
  assert.equal(fs.readFileSync(service.settingsFile, "utf8"), before);
...
test("deleting a manually selected key does not authorize another provider or offline execution", async t => {
  ...
  assert.equal(f.service.updateSettings({ settings:{ provider:"openai", text_scale:"large" } }).ok, true);
  const before = bootstrap.saveRun(f.run);
  const result = await f.service.submitNatural({ ...f.input, text:"Walk toward the open passage." });
  assert.equal(result.ok, false);
  assert.equal(f.requests.length, 0);
...
test("a resolved player action can be narrated without authorizing a second invented action", async t => {
  ...
  assert.equal(bad.result.living_turn.validation.accepted, false);
  assert.equal(bad.result.living_turn.validation.code, "PRESENTATION_PLAYER_AGENCY_INVENTED");
  assert.doesNotMatch(bad.result.scene.narration, /You decide/);
});
...
test("past observable movement does not authorize new movement narration after LOOK", async t => {
  ...
  const result = await executeLivingTurn({ run:f.run, player_text:"Look around.", interpreter:base,
    presentation_provider:{ present:async () => ({ version:PRESENTATION_VERSION, scene_description:"You move toward the open passage.", npc_presentations:[], presentation_claims:[] }) } });
  assert.equal(result.status, "resolved");
  assert.equal(result.resolution.action, "LOOK");
  assert.equal(result.validation.code, "PRESENTATION_PLAYER_AGENCY_INVENTED");
});

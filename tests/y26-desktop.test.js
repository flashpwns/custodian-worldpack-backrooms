...
test("first-run renderer routes creation to the live program selector", () => {
  const renderer = fs.readFileSync(path.join(__dirname, "../desktop/renderer/renderer.js"), "utf8");
  assert.match(renderer, /placeholder="Optional/); assert.doesNotMatch(renderer, /name=\\"name\\" required/); assert.match(renderer, /nameInput\?\.focus\(\)/); assert.match(renderer, /personnel record could not be saved/); assert.match(renderer, /current\.world = result\.world; await selectWorld\(result\.world\.id\)/); assert.doesNotMatch(renderer, /name="experience"/); assert.match(renderer, /action\.startsWith\("mode:"\)\) enterMode\(action\.slice\(5\)\)/); assert.doesNotMatch(renderer, /action\.startsWith\("mode:"\)\) enterMode\(action\.slice\(6\)\)/);
});

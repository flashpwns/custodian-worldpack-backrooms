...
test("radio check is a visible, persisted procedure before field departure", () => {
  ...
  assert.equal(projection.q4.channels.standard.available, true);
  const radioHistory = projection.q4.channels.standard.history.slice(-2);
  assert.deepEqual(new Set(radioHistory.map((item) => item.speaker)), new Set(["You", "STANDARD"]));
  assert.match(radioHistory.find((item) => item.speaker === "You").text, /Clear-Q4 team accounted for/);
  assert.match(radioHistory.find((item) => item.speaker === "STANDARD").text, /acknowledgment received/);

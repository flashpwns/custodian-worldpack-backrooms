...
Status: post-Mission State and Condition-Driven Objectives milestone

Audit date: 2026-08-01

Acceptance evidence: [`acceptance/MISSION_STATE_ACCEPTANCE.html`](acceptance/MISSION_STATE_ACCEPTANCE.html), [`acceptance/MISSION_STATE_EVIDENCE.json`](acceptance/MISSION_STATE_EVIDENCE.json), and the retained structured-interaction and playable-spine evidence

## Executive finding

Yellow Beast now supports an honest, persistent mission loop:

> Receive an assignment -> inspect and alter the authored field -> capture and report evidence -> meet or miss communication, route, personnel, and equipment conditions -> begin return or abort -> reconcile at the declared return point -> receive a derived outcome -> resume the exact final record after restart.

Clear-Q4 now uses one generic mission runtime with nine worldpack-authored objectives, dependency/blocking behavior, objective histories, truthful return readiness, controlled abort, and clean, optional-enhanced, degraded, failed, or aborted results. The largest remaining blockers to a complete game are a general field inventory/custody loop, authored hazards and player-driven personnel/equipment consequences, richer team agency, and bounded Standard/institutional response.
...
- Dynamic mission, spatial, and object state is persisted in `yellow-beast-save@v7` inside `yellow-beast-session@5`; explicit v1–v6/v1–v4 migration retains prior objective, evidence, check-in, route, position, equipment, and radio state.
...
| NPC agency | Cosmetic/partial | Coworkers have canonical identity, location, cognition-backed dialogue, and team-follow behavior | They do not initiate movement, refuse, lag, or pursue goals; blocker GRA-04 |
...
| Communication systems | Functional foundation | Authoritative LOCAL proximity, visible radio check, eight-state radio model, histories, delivered reports | Standard response after the initial check is shallow; blocker GRA-06 |
| Time | Functional foundation | Persisted operational intervals; movement and wait advance time | Few world systems consume time beyond check-ins; blocker GRA-06 |
| Check-ins | Functional foundation | Scheduled/due/overdue/completed/missed/waived semantics; mission objective and debrief consequences; persistence | Missing escalation and richer Standard response; blocker GRA-06 |
| Hazards | Isolated/partial | Spatial schema supports route/location hazard state; anomaly/phenomena systems exist elsewhere | Clear-Q4 has no active hazard resolution in the playable slice; blocker GRA-03 |
...
| Consequences | Isolated/partial | Continuity reviews, scars, equipment/personnel status, trajectory escalation, consequence echoes | Most live field choices do not branch into durable consequences; blocker GRA-03 |
| Failure states | Functional foundation | Controlled abort, missed-check-in degradation, equipment-loss degradation, optional outcomes, unreconciled missing-person failure, and structured final results | Live hazard-driven causes remain blocker GRA-03 |
| Injury/personnel loss | Isolated | Canonical statuses, missing/death distinction, succession and continuity tests | No live injury model, treatment, or player-facing incident resolution; blocker GRA-03 |
...
| Persistent world state | Functional foundation | Atomic writes; canonical mission/history/result, spatial, object, evidence, equipment, radio, team, and time state; v1-v5 session and v1-v7 run restore | Broader schema migration fixtures should continue as systems evolve |
...
| Automated tests | Strong foundation | Playable-spine and interaction gates plus 13 focused mission tests and renderer-backed end-to-end acceptance covering schema, all sources, outcomes, migration, and restart | Live hazard branches remain deferred |
...
- **Remaining extension:** Containers, loose-object custody, route locks, and hazards can use the same affordance/mutation contract in later milestones.
...
### GRA-03 — Hazards, consequences, and recoverable failure

- **Current behavior:** Schemas and isolated systems represent hazards, missing/dead personnel, scars, failed returns, and consequences, but normal Clear-Q4 movement does not invoke them.
- **Desired behavior:** Known and unknown hazards impose choices, checks, resource costs, injury/separation/equipment damage, institutional consequences, and recoverable failure states.
- **Likely subsystem:** Spatial traversal, consequence resolution, personnel/equipment, time, continuity, and worldpack hazard definitions.
- **Implementation direction:** Start with deterministic authored hazards and explicit choices. Use bounded outcomes and seeded variation only where the player has evidence to reason from.
...
### GRA-04 — Team agency and separation

- **Current behavior:** Nora and Alex follow the player as a cohesive team and can answer LOCAL checks. Separation state is representable but generally test-injected.
- **Desired behavior:** Coworkers can accept/refuse instructions, arrive late, remain behind, split, report, lose contact, and update last-known positions without omniscient disclosure.
- **Likely subsystem:** `q4-personnel.js`, cognition, spatial runtime team policy, communication, time.
- **Implementation direction:** Add small deterministic behavior policies and event-driven decisions; no general-purpose pathfinding is required initially.
...
### GRA-06 — Operational time and institutional response

- **Current behavior:** Radio and check-in state are coherent and persisted, but Standard usually records a transmission without a substantive simulated response.
- **Desired behavior:** Standard acknowledges, queries, redirects, warns, authorizes, or records noncompliance according to time, mission, evidence, and communications.
- **Likely subsystem:** `q4-radio.js`, `q4-time.js`, interactions, missions, continuity, institutional simulation.
- **Implementation direction:** Add a bounded Standard procedure/state policy with scheduled events and observer-safe response templates; connect results to review and follow-up assignments.
...
### GRA-11 — Complete Clear-Q4 scenario content

- **Current behavior:** The team can enter the utility room, map and traverse the corridor/passage/lower transition, and encounter a legitimately blocked Level 2 boundary. There is no complete mission arc beyond this foundation.
- **Desired behavior:** Clear-Q4 has an authored beginning, investigation, complication, decision, consequence, return/debrief, and at least one alternate outcome.
- **Likely subsystem:** Clear-Q4 worldpack data/content plus every gameplay subsystem above.
- **Implementation direction:** Build a compact vertical slice over the existing topology, expanding only as required by gameplay beats. Keep unknown space unresolved.
...
Full combat, general autonomous pathfinding, large procedural map generation, cinematic media production, large lore-map expansion, network telemetry, and broad content randomization are not needed for the next playable milestone. The highest-priority next slice is the general equipment/custody/container loop because mission degradation can now consume truthful equipment outcomes; authored hazards and team consequences should follow over that foundation.

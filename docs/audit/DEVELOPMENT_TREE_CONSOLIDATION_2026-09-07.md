...
Fresh dependency installation completed. Aggregate: 617 tests passed, zero failed, plus three required reports. Long-world: 169 tests passed plus its required report. Inventory: 117 included, four quarantined, zero errors or warnings. Explicit asset whitelist and whitespace checks passed. Fresh macOS desktop packaging, packaged offline service smoke, and packaged first-run/relaunch checks passed. The packaged renderer smoke remains FAILED: its simulated provider dropdown selection leaves AUTO selected, so its subsequent offline movement assertion remains at Utility Room. This harness failure is explicitly left for follow-up; no fully-green native-tier claim is made. macOS packaging remains unsigned, and this pass makes no Windows runtime or release-certification claim.
